<?php if (!defined('THINK_PATH')) exit(); /*a:8:{s:56:"D:\webroot\mengdie_yyg\app\www_tpl\default\pay\cart.html";i:1468303703;s:57:"D:\webroot\mengdie_yyg\app\www_tpl\default\base/base.html";i:1470730361;s:63:"D:\webroot\mengdie_yyg\app\www_tpl\default\base/common_css.html";i:1468303703;s:61:"D:\webroot\mengdie_yyg\app\www_tpl\default\base/tool_bar.html";i:1470730313;s:59:"D:\webroot\mengdie_yyg\app\www_tpl\default\base/header.html";i:1468303703;s:59:"D:\webroot\mengdie_yyg\app\www_tpl\default\base/footer.html";i:1470726624;s:61:"D:\webroot\mengdie_yyg\app\www_tpl\default\base/base_url.html";i:1468303703;s:62:"D:\webroot\mengdie_yyg\app\www_tpl\default\base/common_js.html";i:1468303703;}*/ ?>
<!doctype html>
<html lang="zh-cn">
<head>
    <meta charset="UTF-8">
...
<div class="g-body">
    <div class="m-cart main_content">
        <div class="m-header f-clear">
            <div class="m-header-logo">
                <h1>
                    <a class="m-header-logo-link" href="#"><?php echo C('WEBSITE_NAME'); ?></a>
                </h1>
            </div>
                ...
                <div id="content_area">
                    <div id="pro-view-11">
                        <ul class="m-cart-list" id="content_ul">
                            ...
                            <li class="m-cart-list-items f-clear cart_li check_area" data-nper_id="<?php echo $vo['id']; ?>">
                                <div class="cart-list-items-content items-content-checkbox">
                                <span>
                                    <label class="w-checkbox">
                                        <input checked="checked" type="checkbox" class="sel_one " data-id="<?php echo $vo['id']; ?>"/>
                                    </label>
                                </span>
                                </div>
                                <div class="cart-list-items-content items-content-gpic">
                                    <img src="__UPLOAD_DOMAIN__<?php echo (isset($vo['i_img_path']) && ($vo['i_img_path'] !== '')?$vo['i_img_path']:'__yyg__/images/empty_img.jpg'); ?>"
                                         alt="<?php echo (isset($vo['g_name']) && ($vo['g_name'] !== '')?$vo['g_name']:'--'); ?>">
                                </div>
                                <div class="cart-list-items-content items-content-gname">
                                    <!--十元区LOGO-->
                                    <p><a target="_blank" href="#"
                                          class="w-tag w-tag-ten">  <?php if($vo['nper_type'] == 1): echo (isset($vo['deposer_type']) && ($vo['deposer_type'] !== '')?$vo['deposer_type']:'--'); else: ?>
                                        0元抢宝
                                        <?php endif; ?></a>
                                        <a href="<?php echo dwz_filter('goods/detail',array('id'=>$vo['g_id'].'-'.$vo['nper_id'])); ?>"
                                           target="_blank"><?php echo (isset($vo['g_name']) && ($vo['g_name'] !== '')?$vo['g_name']:'--'); ?></a></p>
                                    <p>总需
                                        <span style="color: #39f;"><?php echo intval((isset($vo['sum_times']) && ($vo['sum_times'] !== '')?$vo['sum_times']:'')); ?></span>
                                        人次参与，还剩：
                                    <span class="rest-num">
                                    </span>
                                        <?php if($vo['nper_type'] == 1): echo $vo['sum_times']-$vo['participant_num']; else: if($vo['join_type'] == 1): echo $vo['sum_times']-$vo['odd_join_num']; else: echo $vo['sum_times']-$vo['even_join_num']; endif; endif; ?>
                                        人次
                                    </p>
                                </div>
                                <div class="cart-list-items-content items-content-price f-cart-item-center">
                                    <p><?php echo intval($vo['price']); echo C('MONEY_UNIT'); echo C('MONEY_NAME'); ?></p>
                                </div>
                                <div class="cart-list-items-content items-content-buyunit f-cart-item-center">
                                    <p><?php echo ceil($vo['unit_price']); echo C('MONEY_UNIT'); echo C('MONEY_NAME'); ?></p>
                                </div>
                                <?php if($zero_start == 1): ?>
                                <div class="cart-list-items-content items-content-type f-cart-item-center">
                                    <p> <?php if($vo['nper_type'] == 1): ?>
                                        --
                                        <?php else: if($vo['join_type'] == 1): ?>
                                        奇数抢宝
                                        <?php else: ?>
                                        偶数抢宝
                                        <?php endif; endif; ?></p>
                                ...
                                <span class="items-content-numberInput-number">
                                 <div class="w-number" id="pro-view-13">
                                     ...
                                     <input class="w-number-input participation_num"
                                            type="text"
                                            data-nper="<?php echo (isset($vo['nper_id']) && ($vo['nper_id'] !== '')?$vo['nper_id']:''); ?>"
                                            data-last='<?php if($vo['nper_type'] == 1): echo $vo['sum_times']-$vo['participant_num']; else: if($vo['join_type'] == 1): echo $vo['sum_times']-$vo['odd_join_num']; else: echo $vo['sum_times']-$vo['even_join_num']; endif; endif; ?>'
                                            data-unit="<?php echo (isset($vo['unit_price']) && ($vo['unit_price'] !== '')?$vo['unit_price']:1); ?>"
                                            data-min="<?php echo (isset($vo['min_times']) && ($vo['min_times'] !== '')?$vo['min_times']:1); ?>"
                                            value="<?php echo (isset($vo['num']) && ($vo['num'] !== '')?$vo['num']:1); ?>"/>
                                ...
                                <div class="cart-list-items-content items-content-total f-cart-item-center w-user1s">
                                    <span><em class="money_count">--</em><?php echo C('MONEY_UNIT'); echo C('MONEY_NAME'); ?></span>
                                </div>
                                <div class="cart-list-items-content items-content-operation f-cart-item-center">
                                    <a href="javascript:void(0)" class="del_one" data-nper_id="<?php echo $vo['id']; ?>">删除</a>
                                </div>
                            </li>
                        ...
                        <div class="m-cart-list-footer f-clear ">
                            ...
                            <div id="pro-view-21" class="no_goods_hide">
                                <div class="cart-list-footer-total">
                                    <p class="txt-bold">总计：<strong id="total_amount">--</strong>&nbsp;<?php echo C('MONEY_UNIT'); echo C('MONEY_NAME'); ?></p>
                                    <p class="txt-gray balance">账户余额：<?php echo (isset($money) && ($money !== '')?$money:0); ?>&nbsp;<?php echo C('MONEY_UNIT'); echo C('MONEY_NAME'); ?></p>
                                </div>
...
<input type="hidden" id="show_rand_goods_url" value="<?php echo U('Pay/show_rand_goods'); ?>"><!--显示随机商品url-->
<input type="hidden" id="pay_info_url" value="<?php echo U('Pay/info'); ?>"><!--确认商品信息-->
<input type="hidden" id="charge_url" value="<?php echo U('Pay/charge'); ?>"><!--商品付款页面地址-->
...
<script type="text/javascript" src="__yyg__/js/pay/cart.js"></script>
