...
Class Ucenter extends Common
{
    ...
    public function index()
    {
        ...
        list($result, $count_all,,$num) = $m_order->get_payed_order_by_user($this->user_info['id'], 1, 20, null);
...
//    宝记录-已揭晓
    ...
    public function charge()
    {
        ...
        $info = $m_user->get_login_user_info();
        //获取历史充值总额
        $total_price = $m_uc->get_total_charge();
  ...
  //提现检验
    ...
    public function get_money_by_cash(){
       $u_model=new UserModel();
        $rt = $u_model->get_money_by_cash(I('post.money'));
        $rt && ok_return('转换成功!');
        wrong_return('转换失败');
    }
    //TA的晒单
