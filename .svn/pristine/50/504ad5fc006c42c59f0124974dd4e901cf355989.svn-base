...
class RewardModel {
    ...
    //获取最新推广记录5条
    public function get_last_rewards($uid=""){
        if(empty($uid)){
            $uid = get_user_id();
        }
        return $this->m->alias('r')->field('r.*,u.username')->join('sp_users u ON r.uid=u.id')->where(['spread_uid'=>$uid,'r.status'=>1])->order('r.create_time desc')->limit(6)->select();
    }
    //获取下级消费总金额
    public function get_total($post){
        return $this->m->where('status=1 AND spread_uid='.get_user_id().$post->wheresql)->sum('total');
    }
    //获取推广收益
    public function get_total_reward($post){
        return $this->m->where('status=1 AND spread_uid='.get_user_id().$post->wheresql)->sum('reward');
    }
    //按照用户级别
    public function get_rewards_group_by_level($post){
        ...
        FROM  sp_reward
        WHERE  status=1 AND spread_uid=".get_user_id() . $post->wheresql .
    ...
    public function get_register_rewards_by_id($post){
        ...
        WHERE  r.status=1 AND r.spread_uid=".get_user_id() . $post->wheresql .
            " ORDER BY create_time DESC " . $post->limitData;
        $sql_count = "SELECT FOUND_ROWS() as num";
    ...
    public function m_register_rewards($uid){
        $r_model=M('reg_reward','sp_');
        return $r_model->alias('r')->field('r.*,u.username')->join('sp_users u ON u.id=r.uid','left')->where(['r.spread_uid'=>$uid,'r.status'=>1])->order('r.create_time DESC')->limit(0,10)->select();
    }
    //获取总页码
    public function m_register_pages($uid){
        $r_model=M('reg_reward','sp_');
        $total=$r_model->field('id')->where(['spread_uid'=>$uid,'status'=>1])->count();
    ...
    public function m_promote_pages($level,$uid){
        $total=count($this->m->where(['spread_uid'=>$uid,'level'=>$level,'status'=>1])->group('uid,month_time')->select());
        return ceil($total/10);
    }
    //
    public function m_ajax_promote_detail_list($uid,$offset=0,$year='',$month='',$level){
        ...
        $total=$this->m->alias('r')->field('sum(total) as total,sum(reward) as reward')->where($where)->select();
    ...
    public function m_promote_detail($level,$uid){
        $data['list']= $this->m->alias('r')
            ...
            ->join('sp_users u on u.id=r.uid')->where(['r.spread_uid'=>$uid,'r.level'=>$level,'r.status'=>1])
            ->group('r.month_time,r.uid')->order('r.month_time desc')->limit(10)->select();
        $total=$this->m->field('sum(total) as total,sum(reward) as reward')->where(['spread_uid'=>$uid,'level'=>$level,'status'=>1])->select();
        $data['total_fee']=$total[0]['total'];$data['total_reward']=$total[0]['reward'];
        return $data;
    }
    //获取分销推广汇总
